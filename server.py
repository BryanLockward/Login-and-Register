...
def returnDashboard():
    if session['loggedin'] == True:
        ...
        return render_template('sucess.html', posts=postData)
    else:
        return redirect('/')
...
def postData():
    if session['loggedin'] == True:
        ...
        query = "INSERT INTO messages (post_text, created_at, updated_at, user_id) VALUES ('{}', NOW(), NOW(), '{}')".format(postMessage, session['userid'])
        mysql.run_mysql_query(query)
        return redirect('/dashboard')
    else:
        return redirect('/')
